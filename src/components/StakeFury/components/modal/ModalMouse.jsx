...
import ABI from "components/StakeFury/components/card/ABI/ABI";
import VAULTABI from "components/StakeFury/components/card/ABI/VAULTABI";
...
async function fetchNFTsForContract() {
  ...
  async function stake() {
    ...
    try {
      ...
      const tx = await vaultcontract.stake_furyMice(houses, numberArray, {
        from: account
      })
      setSuccess(true);
    } catch (e) {
    ...
    <>
      ...
      >
        ...
        <div style={{alignItems:'center', marginTop: 10, marginBottom: 10,  display: "flex", textAlign: 'center', color: 'white'}}>
          ...
          >
              {NFTs.map((tokenId) => {
                    const image = `https://ipfs.io/ipfs/QmYKQHVhrzN87WAx5iQUvvnYCCFatMZKVFafomrYVEwxiC/${tokenId}.webp`
